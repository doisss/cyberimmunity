...
**Описание работы:** задача — спроектировать и реализовать простейший онлайн-магазин при помощи высокоуровневых средств разработки (Django).
...
3. Используя заготовленную версию Django-сервера (директория django-test в корневой директории проекта; там же есть файл с зависимостями проекта - requirements.txt) реализовать все функции, указанные выше.
...
Административная панель использует карту (Mapbox) для отображения точки доставки. Токен доступа в собранном фронтенде (`django-test/server/static/static/js/main.7b277d84.js`) заменён на заглушку `YOUR_MAPBOX_TOKEN_HERE`, поэтому карта по умолчанию не отображается.

Чтобы карта заработала, нужно подставить свой Mapbox-токен (получить бесплатно на [account.mapbox.com](https://account.mapbox.com/) → Tokens) — заменить `YOUR_MAPBOX_TOKEN_HERE` на свой токен в указанном файле, либо пересобрать фронтенд из исходников с токеном, заданным через переменную окружения.