...
class FileLogger:
    def __init__(self, base_filename="log"):
        ...
        self.main_file = f"{self.base_filename}1.txt"
        self.line_count = 0

        # очистим основной файл при старте
    ...
    def log(self, text: str):
        ...
        # каждая 3-я строка → создаём новый снапшот
        if self.line_count % 3 == 0:
            snapshot_index = (self.line_count // 3) + 1
            snapshot_file = f"{self.base_filename}{snapshot_index}.txt"
            with open(self.main_file, "r", encoding="utf-8") as src:
                content = src.read()
            with open(snapshot_file, "w", encoding="utf-8") as dst:
                dst.write(content)
            print(f"[logger] sozdali snapshot: {snapshot_file}")
...
class Monitor(Process):
    ...
    def add_entity_queue(self, entity_id: str, queue: Queue):
        logger.log(f"[monitor] registriruem sushnost {entity_id}")
        self._entity_queues[entity_id] = queue

    def _check_policies(self, event):
        logger.log(f'[monitor] obrabatyvaem sobytie {event}')
        ...
        if event.operation == "set_mode":
            try:
                ...
                if mode in traffic_lights_allowed_configurations:
                    authorized = True
                    logger.log("[monitor] politika bezopasnosti: dostup razreshen")
            except Exception:
                pass
        ...
        if authorized is False:
            logger.log("[monitor] sobytie ne razresheno politikami bezopasnosti")

        return authorized

    def _proceed(self, event):
        logger.log(f'[monitor] peresylaem sobytie {event}')
        try:
            dst_q: Queue = self._entity_queues[event.destination]
            dst_q.put(event)
        except Exception as e:
            logger.log(f"[monitor] oshibka peresylki sobytiya {e}")

    def run(self):
        logger.log(f'[monitor] start')
        while self._force_quit is False:
            ...
            try:
                ...
                logger.log(f"[monitor] oshibka obrabotki {e}, {event}")
            self._check_control_q()
        logger.log(f'[monitor] stop')
...
class ControlSystem(Process):
    ...
    def run(self):
        logger.log(f'[ControlSystem] start')
        ...
        while True:
            ...
            logger.log(f'[ControlSystem] otpravlyaem rezhim: {mode}')
                ...
                try:
                    ...
                    if event_in.operation == "city_command":
                        ...
                        if "green_duration" in cmd:
                            ...
                            logger.log(f"[ControlSystem] poluchena komanda iz goroda: izmenit green_duration na {self.green_duration}")
                        if "set_mode" in cmd:
                            mode = cmd["set_mode"]
                            logger.log(f"[ControlSystem] poluchena komanda iz goroda: perehod v rezhim {mode}")
...
class LightsGPIO(Process):
    ...
    def run(self):
        logger.log(f'[LightsGPIO] start')
        while True:
            try:
                event: Event = self._own_queue.get_nowait()
                if event.operation == "set_mode":
                    mode = json.loads(event.parameters)
                    logger.log(f"[LightsGPIO] noviy rezhim svetofora: {mode}")
            except Empty:
                sleep(0.2)
...
class CitySystemConnector(Process):
    ...
    def run(self):
        logger.log(f'[CitySystemConnector] start')
        ...
        logger.log(f"[CitySystemConnector] otpravlyaem komandu: {cmd}")
        self.monitor_queue.put(event)
        ...
        while True:
            try:
                ...
                if event.operation == "diagnostic":
                    ...
                    logger.log(f"[CitySystemConnector] diagnostika: svetofor = {data}")
            except Empty:
                sleep(0.2)
